...
.libPaths()
.libPaths(new = c("/home/manuel.tardaguila/conda_envs/ORA/lib/R/library"))
.libPaths()
...
ORA_function = function(option_list)
{
  ...
  out_path <- paste0(out,'/','ORA','_','background_adapted/') # output path, where you want your results exported to
...
for(k in 1:length(array_contrasts)){
  ...
  DE_results_NO_NA_SIG_contrast_sel<-DE_results_NO_NA_SIG[which(DE_results_NO_NA_SIG$contrast == contrast_sel),]
  
  if(dim(DE_results_NO_NA_SIG_contrast_sel)[1] > 0){
    
    if(DEBUG ==1){
      
      cat("DE_results_NO_NA_SIG_contrast_sel_0\n")
      cat(str(DE_results_NO_NA_SIG_contrast_sel))
      cat("\n")
    }
    
    # Split the dataframe into a list of sub-dataframes: upregulated, downregulated genes
    
    deg_results_list <- split(DE_results_NO_NA_SIG_contrast_sel, DE_results_NO_NA_SIG_contrast_sel$diffexpressed)
    ...
    if(dim(contrast_df)[1] >0){
      ...
      ORA_df<-rbind(contrast_df,ORA_df)
  ...
  }# dim(DE_results_NO_NA_SIG_contrast_sel)[1] > 0
...
if(dim(ORA_df)[1] >0){
  ...
  write.table(ORA_df_SIG, file="ORA_results_significant.tsv", sep="\t", quote=F, row.names = F)
  

  saveRDS(ORA_df_SIG, file="ORA_results_significant.rds")
...
Lolliplot_and_gene_annotation = function(option_list)
{
  ...
  ORA_result.dt<-data.table(ORA_result, key=c("ID","contrast","diffexpressed"))
  ...
  indx<-c(which(colnames(ORA_result_MAX)%in%c('ID','diffexpressed','geneID','PATH_class')))
  ...
  ORA_result_MAX_sub_long.dt<-data.table(ORA_result_MAX_sub_long, key=c("gene","diffexpressed","PATH_class"))
  ...
  gene_annotation_wide<-as.data.frame(pivot_wider(gene_annotation, id_cols=c("gene","diffexpressed"), names_from=PATH_class, values_from=string_ID), stringsAsFactors=F)
  ...
  if(dim(ORA_result_MAX_Thresholded)[1] >0){
    
    if(DEBUG == 1){
      
      cat("ORA_result_MAX_Thresholded_0\n")
      cat(str(ORA_result_MAX_Thresholded))
      cat("\n")
    }
    
    array_diffexpressed<-unique(ORA_result_MAX_Thresholded$diffexpressed)
    ...
    if(length(array_diffexpressed) >0){
      ...
      for(iteration_diffexpressed in 1:length(array_diffexpressed)){
        ...
        directionality_df<-ORA_result_MAX_Thresholded[which(ORA_result_MAX_Thresholded$diffexpressed == direction_sel),]
        ...
        if(dim(directionality_df)[1] >0){
          ...
          if(FLAG_over_represent <= 50){
            ...
            REP<-directionality_df
            
            
          }else{
            
            directionality_df<-directionality_df[order(directionality_df$minuslog10padj, decreasing = T),]
            
            
            selected_pathways<-unique(as.character(directionality_df$ID[c(1:50)]))
            
            REP<-directionality_df[which(directionality_df$ID%in%selected_pathways),]
          ...
          ORA_lolliplot<-ggplot(data=REP, 
                                aes(y=as.numeric(DUMMY),
                                    x=minuslog10padj)) +
            geom_segment(data=REP,
                         aes(y=as.numeric(DUMMY),
  ...
  }#dim(ORA_result_MAX_Thresholded)[1] >0
