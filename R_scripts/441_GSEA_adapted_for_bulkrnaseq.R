...
GSEA_function = function(option_list)
{
  ...
  out_path <- paste0(out,'/','GSEA','_','background_adapted/') # output path, where you want your results exported to
  ...
  #### READ and transform sample_file ----
  
  
  sample_file<-read.table(file=opt$sample_file, sep="\t", header=T)
  
  cat("sample_file_0\n")
  cat(str(sample_file))
  cat("\n")
  cat(sprintf(as.character(names(summary(as.factor(sample_file$contrast))))))
  cat("\n")
  cat(sprintf(as.character(summary(as.factor(sample_file$contrast)))))
  cat("\n")
  
  ##### read all files to create a DE_table ----
  
  DE_results<-data.frame()
  
  for(i in 1:dim(sample_file)[1]){
    
    sample_file_sel<-sample_file[i,]
    
    file_sel<-sample_file_sel$File
    
    contrast_sel<-sample_file_sel$Contrast
    
    
    selected_table<-read.table(file=file_sel, sep=",", header=T)
    
    
    cat("selected_table_0\n")
    cat(str(selected_table))
    cat("\n")
    
    selected_table$contrast<-contrast_sel
    
    colnames(selected_table)[which(colnames(selected_table) == "X")]<-"gene"
    
    DE_results<-rbind(selected_table,DE_results)
    
  }#i in 1:dim(sample_file)[1]
  ...
  DE_results$contrast<-factor(DE_results$contrast,
                              levels=sample_file$Contrast,
                              ordered=T)
  
  cat("DE_results_1\n")
  cat(str(DE_results))
  cat("\n")
  cat(sprintf(as.character(names(summary(DE_results$contrast)))))
  cat("\n")
  cat(sprintf(as.character(summary(DE_results$contrast))))
  cat("\n")
  
  setwd(out)
  
  saveRDS(DE_results, file="DE_results.rds")
  ...
  for(k in 1:length(array_contrasts)){
    ...
    DE_results_NO_NA_with_ENTREZ_sel<-DE_results_NO_NA_with_ENTREZ[which(DE_results_NO_NA_with_ENTREZ$contrast == contrast_sel),]
    ...
    if(dim(DE_results_NO_NA_with_ENTREZ_sel)[1] > 0){
      ...
      if(dim(contrast_df)[1] >0){
        ...
        GSEA_df<-rbind(contrast_df,GSEA_df)
        
        Leading_edge_list[[k]]<-List_bg_files
        
        names(Leading_edge_list)[k]<-contrast_sel
  ...
  if(dim(GSEA_df)[1] >0){
    ...
    write.table(GSEA_df_SIG, file="GSEA_results_significant.tsv", sep="\t", quote=F, row.names = F)
    
    saveRDS(Leading_edge_list, file="GSEA_complete_results.rds")
    
    saveRDS(GSEA_df_SIG, file="GSEA_results_significant.rds")
...
Leading_edge_printer = function(option_list)
{
  ...
  DE_results<-readRDS(file="DE_results.rds")
  ...
  indexes_contrasts<-names(List_GSEA)
  ...
  if(length(indexes_contrasts_NO_NA_NO_EMPTY) >0){
    for(k in 1:length(indexes_contrasts_NO_NA_NO_EMPTY)){
      ...
      path_contrast_sel<-paste(out,contrasts_sel,'/',sep='')
      ...
      List_GSEA_contrast_sel<-List_GSEA[[indexes_contrasts_NO_NA_NO_EMPTY_sel]]
      
      
      
      array_collections<-names(List_GSEA_contrast_sel)
      ...
      for(h in 1:length(array_collections)){
        ...
        List_GSEA_contrast_sel_collection_sel<-List_GSEA_contrast_sel[[array_collections_sel]]
        
        array_gene_sets<-List_GSEA_contrast_sel_collection_sel$Description
        ...
        for(l in 1:length(array_gene_sets)){
          ...
          Leading_edge_plot<-gseaplot(List_GSEA_contrast_sel_collection_sel, geneSetID = l, title = List_GSEA_contrast_sel_collection_sel@result$Description[l])
...
Lolliplot_and_gene_annotation = function(option_list)
{
  ...
  # Maximize pvalue by ID, contrast -----------------------------------
  
  
  GSEA_result.dt<-data.table(GSEA_result, key=c("ID","contrast"))
  ...
  indx<-c(which(colnames(GSEA_result_MAX_with_count)%in%c('ID','core_enrichment','PATH_class')))
  ...
  GSEA_result_MAX_with_count_sub_long.dt<-data.table(GSEA_result_MAX_with_count_sub_long, key=c('gene','PATH_class'))
  ...
  gene_annotation_wide<-as.data.frame(pivot_wider(gene_annotation, id_cols='gene', names_from=PATH_class, values_from=string_ID), stringsAsFactors=F)
  ...
  if(dim(GSEA_result_MAX_with_count_Thresholded)[1] >0){
    ...
    FLAG_over_represent<-length(levels_ID)
    
    if(FLAG_over_represent <= 50){
      
      REP<-GSEA_result_MAX_with_count_Thresholded
      
      
    }else{
      
      GSEA_result_MAX_with_count_Thresholded<-GSEA_result_MAX_with_count_Thresholded[order(GSEA_result_MAX_with_count_Thresholded$minuslog10padj, decreasing = T),]
      
      selected_pathways<-unique(as.character(GSEA_result_MAX_with_count_Thresholded$ID[c(1:50)]))
      
      REP<-GSEA_result_MAX_with_count_Thresholded[which(GSEA_result_MAX_with_count_Thresholded$ID%in%selected_pathways),]
      
    }
    
    REP$DUMMY<-droplevels(factor(REP$ID, levels=levels_ID, ordered=T))
    ...
    breaks_gene_sets<-as.numeric(REP$DUMMY)
    labels_gene_sets<-as.character(gsub("\\..+$","",REP$DUMMY))
    
    
    Gene_set_lolliplot<-ggplot(data=REP, 
                               aes(y=as.numeric(DUMMY),
                                   x=minuslog10padj)) +
      geom_segment(data=REP[which(REP$minuslog10padj > 0), ],
                   aes(y=as.numeric(DUMMY),
                       ...
                       color=NES),
                   size=0.8)+
      geom_point(data=REP[which(REP$minuslog10padj > 0), ],
                 aes(color=NES),size=5, stroke=1, shape=21,fill="white")+
      geom_text(data=REP[which(REP$minuslog10padj > 0), ],
    ...
    Gene_set_lolliplot <-Gene_set_lolliplot+
      theme_cowplot(font_size = 2,
                    font_family = "sans")+
      facet_grid(. ~ contrast, scales='free_x', space='free_x', switch="y", drop=TRUE)+
  ...
  }# dim(GSEA_result_MAX_with_count_Thresholded)[1] >0
...
Lolliplot_and_gene_annotation_subset = function(option_list)
{
  ...
  if(dim(GSEA_result_MAX_with_count_Thresholded_subset)[1] >0){
    
    GSEA_result_MAX_with_count_Thresholded_subset<-GSEA_result_MAX_with_count_Thresholded_subset[order(GSEA_result_MAX_with_count_Thresholded_subset$PATH_class),]
    
    levels_ID<-unique(as.character(GSEA_result_MAX_with_count_Thresholded_subset$ID))
    
    GSEA_result_MAX_with_count_Thresholded_subset$DUMMY<-factor(GSEA_result_MAX_with_count_Thresholded_subset$ID, levels=levels_ID, ordered=T)
    ...
    breaks_gene_sets<-as.numeric(GSEA_result_MAX_with_count_Thresholded_subset$DUMMY)
    labels_gene_sets<-as.character(gsub("\\..+$","",GSEA_result_MAX_with_count_Thresholded_subset$DUMMY))
    
    
    Gene_set_lolliplot<-ggplot(data=GSEA_result_MAX_with_count_Thresholded_subset, 
                               aes(y=as.numeric(DUMMY),
                                   x=minuslog10padj)) +
      geom_segment(data=GSEA_result_MAX_with_count_Thresholded_subset[which(GSEA_result_MAX_with_count_Thresholded_subset$minuslog10padj > 0), ],
                   aes(y=as.numeric(DUMMY),
                       yend=as.numeric(DUMMY),
                       x=0,
                       xend=minuslog10padj,
                       color=NES),
                   size=0.8)+
      geom_point(data=GSEA_result_MAX_with_count_Thresholded_subset[which(GSEA_result_MAX_with_count_Thresholded_subset$minuslog10padj > 0), ],
                 aes(color=NES),size=5, stroke=1, shape=21,fill="white")+
      geom_text(data=GSEA_result_MAX_with_count_Thresholded_subset[which(GSEA_result_MAX_with_count_Thresholded_subset$minuslog10padj > 0), ],
                aes(x=minuslog10padj, y=as.numeric(DUMMY), label=Count),color="black",size=2, family="sans",fontface="bold")+
      geom_segment(data=GSEA_result_MAX_with_count_Thresholded_subset[which(GSEA_result_MAX_with_count_Thresholded_subset$minuslog10padj < 0), ],
                   aes(y=as.numeric(DUMMY),
                       yend=as.numeric(DUMMY),
  ...
  }# dim(GSEA_result_MAX_with_count_Thresholded_subset)[1] >0
